    ...
    def delete(usuario, controller, idTarefa):
        try:
            ...
            if not tarefa_deletada:
                return Response(
                    json.dumps(ErroDTO(400, "Não foi possível realizar sua requisição, tente novamente").__dict__),
                    status=400,
                    mimetype='application/json'
                )
        ...
        def get(usuario, controller):
            try:
                ...
                if 'status' in request.args \
                    ...
                    erros.append("Campo 'status' com opção inválido. Favor informar um status 0, 1 ou 2.")
